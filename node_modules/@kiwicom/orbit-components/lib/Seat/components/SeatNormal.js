"use strict";

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");

var _interopRequireWildcard = require("@babel/runtime/helpers/interopRequireWildcard");

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = exports.StyledStrokeNormal = exports.StyledPath = void 0;

var React = _interopRequireWildcard(require("react"));

var _styledComponents = _interopRequireDefault(require("styled-components"));

var _defaultTheme = _interopRequireDefault(require("../../defaultTheme"));

var _useTheme = _interopRequireDefault(require("../../hooks/useTheme"));

var _consts = require("../consts");

var _helpers = require("./helpers");

var StyledPath = _styledComponents.default.path.withConfig({
  displayName: "SeatNormal__StyledPath",
  componentId: "td69uu-0"
})(["transition:fill ", " ease-in;"], function (_ref) {
  var theme = _ref.theme;
  return theme.orbit.durationFast;
}); // $FlowFixMe: https://github.com/flow-typed/flow-typed/issues/3653#issuecomment-568539198


exports.StyledPath = StyledPath;
StyledPath.defaultProps = {
  theme: _defaultTheme.default
};

var StyledStrokeNormal = _styledComponents.default.path.withConfig({
  displayName: "SeatNormal__StyledStrokeNormal",
  componentId: "td69uu-1"
})(["transition:stroke ", " ease-in;"], function (_ref2) {
  var theme = _ref2.theme;
  return theme.orbit.durationFast;
}); // $FlowFixMe: https://github.com/flow-typed/flow-typed/issues/3653#issuecomment-568539198


exports.StyledStrokeNormal = StyledStrokeNormal;
StyledStrokeNormal.defaultProps = {
  theme: _defaultTheme.default
};

var SeatNormal = function SeatNormal(_ref3) {
  var selected = _ref3.selected,
      type = _ref3.type,
      label = _ref3.label;
  var theme = (0, _useTheme.default)();
  if (selected && type === _consts.TYPES.UNAVAILABLE) return null;
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(StyledPath, {
    d: "M1 10C1 5.02944 5.02944 1 10 1H36C40.9706 1 45 5.02944 45 10V42C45 43.6569 43.6569 45 42 45H4C2.34315 45 1 43.6569 1 42V10Z",
    fill: (0, _helpers.resolveFillColor)({
      theme: theme,
      type: type,
      selected: selected
    })
  }), type !== _consts.TYPES.UNAVAILABLE && /*#__PURE__*/React.createElement("text", {
    fill: (0, _helpers.resolveTextColor)({
      theme: theme,
      type: type,
      selected: selected
    }),
    xmlSpace: "preserve",
    fontSize: "16",
    letterSpacing: "0em",
    textAnchor: "middle",
    dominantBaseline: "middle",
    x: "50%",
    y: "55%"
  }, label), type === _consts.TYPES.UNAVAILABLE && /*#__PURE__*/React.createElement("path", {
    d: "M28.6555 17.3331C29.0072 17.6844 29.0077 18.2542 28.6564 18.6059L24.5533 22.7144C24.3973 22.8705 24.3973 23.1235 24.5533 23.2797L28.6564 27.3885C28.9784 27.7109 29.0049 28.2166 28.7361 28.5692L28.6555 28.6613C28.3038 29.0125 27.7339 29.0121 27.3827 28.6603L23.2821 24.5535C23.1258 24.397 22.8722 24.3971 22.7159 24.5536L18.6169 28.6603C18.2657 29.0121 17.6958 29.0125 17.3441 28.6613C16.9923 28.3101 16.9919 27.7402 17.3431 27.3885L21.4458 23.2796C21.6017 23.1235 21.6017 22.8705 21.4458 22.7144L17.3431 18.6059C17.0212 18.2835 16.9947 17.7778 17.2635 17.4252L17.3441 17.3331C17.6958 16.9819 18.2657 16.9823 18.6169 17.3341L22.716 21.4396C22.8722 21.596 23.1258 21.5961 23.2821 21.4396L27.3827 17.3341C27.7339 16.9823 28.3038 16.9819 28.6555 17.3331Z",
    fill: theme.orbit.paletteInkLighterHover
  }), !selected && /*#__PURE__*/React.createElement("path", {
    d: "M0 40H46V42C46 44.2091 44.2091 46 42 46H4C1.79086 46 0 44.2091 0 42V40Z",
    fill: (0, _helpers.resolveAccentColor)({
      theme: theme,
      type: type,
      selected: selected
    })
  }), /*#__PURE__*/React.createElement(StyledStrokeNormal, {
    d: "M1 10C1 5.02944 5.02944 1 10 1H36C40.9706 1 45 5.02944 45 10V42C45 43.6569 43.6569 45 42 45H4C2.34315 45 1 43.6569 1 42V10Z",
    stroke: (0, _helpers.resolveAccentColor)({
      theme: theme,
      type: type,
      selected: selected
    }),
    strokeWidth: "2"
  }));
};

var _default = SeatNormal;
exports.default = _default;